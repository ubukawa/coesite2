...
const vtUser = config.get('userInfo.vtUser') //13 July 2021 
const vtPass = config.get('userInfo.vtPass') //13 July 2021 
...
router.get(`/zxy/:t/:z/:x/:y.pbf`, 
 async function(req, res) {
  ...
  const accessuser =  req.query.user //added
  const passwd =  req.query.password //added

if (accessuser == vtUser && passwd == vtPass) {
  getMBTiles(t, z, x, y).then(mbtiles => {
    ...
    res.status(404).send(`mbtiles not found for /zxy/${t}/${z}/${x}/${y}.pbf`)
        ...
        res.status(404).send(`You are not allow to get: /zxy/${t}/${z}/${x}/${y}.pbf`)
 };
